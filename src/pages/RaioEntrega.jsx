...
// Grade padrão: 7 dias fechados, sem períodos.
...
function normalizaHorarios(raw, aberturaLegado, fechamentoLegado) {
  ...
  // Sem grade salva: se tinha abertura/fechamento único, semeia todos os dias com esse período.
...
export default function RaioEntrega() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('empresas')
      .select('id, aceita_delivery, taxa_entrega, taxas_entrega_km, tempo_entrega_min, tempo_entrega_max, cep, endereco, numero, bairro, cidade, estado, categoria_delivery, horario_abertura, horario_fechamento, horarios_funcionamento, raio_entrega_km, latitude, longitude')
  ...
  // ── Horários de funcionamento (grade semanal, estilo iFood) ──
  ...
  return (
    <form onSubmit={handleSalvar}>
      <div className="re-page">
        ...
        <div className="card">
          <h2 className="re-card-title">Horários de Funcionamento</h2>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: -6, marginBottom: 14 }}>
            Defina os dias e períodos em que a loja fica <strong>aberta</strong> para receber pedidos. Fora desses horários ela aparece como fechada.
          </p>
